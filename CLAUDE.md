...
**Приоритет v1 (см. ROADMAP.md):** сначала голый daily-бот — движок, деплой в прод, полировка ощущений. Без челленджей и без Mini App: сначала отточить, как база работает и чувствуется на живых людях, фичи подключать по одной только после. Не начинать работу над `webapp/` и над челленджами (`/api/challenge`), пока это явно не запрошено.
