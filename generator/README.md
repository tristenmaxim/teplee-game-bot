...
2. `uv run python generate_ranks.py` → `data/game_static.db` (100 дней × RU+EN, фикс-сид шаффл).
3. `uv run python validate.py <слово>` на тестовых словах из §5 (вода, автомобиль, любовь, компьютер, собака, музыка, война, деньги, море, хлеб / water, car, love, computer, dog, music, war, money, sea, bread) — глазами смотришь топ-50. Если мусор — правим фильтры в `build_guess_dict.py` и пересобираем.
