...
```
Один сервис, никакого Caddy. Порт 8000 привязан только к `127.0.0.1` — снаружи недоступен, нужен исключительно для локального health-чека. Порт 8500 проброшен публично (`0.0.0.0:8500:8000`) — только ради `/admin` (см. ниже); `/api/*` через него тоже технически доступен, но защищён своей auth (initData), так что это не новая дыра. Больше никаких `0.0.0.0`-портов наружу (кроме исходящих — long polling сам стучится к Telegram).

⚠️ Порт для публичного доступа выбирай, проверив `ss -tlnp` на сервере — на нём уже висят чужие сервисы на 1500, 1502 (HH Monitor), 3000, 8078, 8090.

`.env` на сервере (не в git, лежит в `/opt/teplee/.env`): `BOT_TOKEN`, `DB_PATH=/data/game.db`, `STATIC_DB_PATH=/data/game_static.db`, `DATA_DIR=/data`, `TZ=Europe/Moscow`, `ADMIN_ID` (для алертов health-чека, см. `healthcheck.sh`). Плюс временно (см. ниже) `GAME_EPOCH_DATE`.

Для `/admin` (редактор текстов бота): `ADMIN_IDS` (через запятую, Telegram user id владельцев панели — обычно тот же id, что в `ADMIN_ID`), `ADMIN_SESSION_SECRET` (случайная строка для подписи сессионной куки), `API_PUBLIC_URL=http://<IP сервера>:8500`. Домен и HTTPS не нужны: авторизация не через Telegram Login Widget, а через ссылку, которую бот сам присылает в ответ на `/admin` в чате (см. `app/admin_auth.py`) — открывается прямо по IP.
