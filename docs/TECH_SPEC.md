...
### Игровое сообщение
- У каждого юзера в ЛС — одно «игровое сообщение» (`users.game_message_id`), контент из PRD §5A + inline-кнопки: `WebAppInfo`-кнопка «🎮 Открыть игру», «🎯 Загадать другу», «🇷🇺/🇬🇧».
- Ответ на попытку (guess) = **новое сообщение** (send, не edit), id сохраняется в `game_message_id`. Изначально попытки делали `editMessageText`, но живое тестирование показало, что edit оставляет сообщение на старой позиции в чате и пользователь его не замечает — теперь каждая попытка пересылает игровое сообщение заново.
- Тумблер языка по кнопке «🇷🇺/🇬🇧» — редкое in-place действие, по-прежнему `editMessageText`; если edit падает (сообщение удалено юзером / >48ч / message is not modified) → отправить новое и сохранить id.
- Троттлинг: не чаще 1 раза в секунду на чат (Telegram limit), очередь per-user — действует и на edit, и на send.
